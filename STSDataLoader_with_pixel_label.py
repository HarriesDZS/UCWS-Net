...
class STSDataLoader(data.Dataset):
    def __init__(self, type="train", transform=None):
        ...
        (train_ids, valid_ids, train_map, valid_map) = self.fetch_train_valid_ids()

        if type == "train":
             (self.data_map, self.idx_map) = self.fetch_data_map(ids=train_ids, maps=train_map)
        else:
            (self.data_map, self.idx_map) = self.fetch_data_map(ids=valid_ids, maps=valid_map)
    ...
    def fetch_train_valid_ids(self):
        ...
        with open(os.path.join(self.project_dir, "STS_train"), "r") as file:
            for line in file:
                ...
                train_map[temp[0]] = [int(temp[1]), int(temp[2])]
        with open(os.path.join(self.project_dir, "STS_valid"), "r") as file:
            for line in file:
                temp = line.strip().split(",")
                valid_ids.append(temp[0])
                valid_map[temp[0]] = [int(temp[1]), int(temp[2])]
        return (train_ids, valid_ids, train_map, valid_map)

    def fetch_data_map(self, ids, maps):
        """
        ...
        for id in ids:
            ...
            pet_file_name = os.path.join(self.root_dir, "STS_{}_PET.npy".format(str(id)))
            label = np.load(file_name)
            pet = np.load(pet_file_name)
            data_map[id] = []
            for i in range(maps[id][0], maps[id][1]+1):
